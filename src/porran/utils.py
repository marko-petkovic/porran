"""General utilities for CIF parsing, coordinate processing, and CIF writing."""
...
def determine_crystal_system(a, b, c, alpha, beta, gamma, digits=3):
    """Infer a crystal-system label from lattice lengths and angles.

    Parameters are rounded to `digits` before comparison to make the
    classification robust to small floating-point noise.
    """
...
def normalize_supercell(supercell):
    """Normalize supercell input to a validated (sx, sy, sz) tuple."""
    ...
    if isinstance(supercell, int):
        ...
        return (supercell, supercell, supercell)

    if len(supercell) != 3:
        raise ValueError("supercell must be an int or a tuple/list with 3 integers")

    sc = tuple(int(x) for x in supercell)
    if any(x < 1 for x in sc):
        raise ValueError("supercell dimensions must be >= 1")
    return sc


def expand_frac_positions(frac_positions: np.ndarray, supercell):
    """Expand fractional coordinates from unit cell to a supercell."""
    ...
    for ix in range(sx):
        for iy in range(sy):
            for iz in range(sz):
                ...
                expanded.append((frac_positions + shift) / np.array(supercell, dtype=float))
    return np.vstack(expanded)

def extract_linkers(download_path: str):
    """Extract linker connected components and their representative positions.

    Parameters
    ----------
    download_path : str
        Directory containing `linkers.cif` generated by MOFid.

    Returns
    -------
    tuple[list[list[int]], list[np.ndarray]]
        Linker atom-index groups and their mean fractional coordinates.
    """
...
def readcif(name):
    """Parse basic lattice parameters, atom types, and fractional positions from a CIF.

    This parser reads `_cell_*` and `_atom_site_*` entries and returns atom
    types as atomic numbers.
    """
...
def process_cif_jimage(string):
    """Convert CIF bond image encoding to a jimage offset list."""
    if string == ".":
        return [0, 0, 0]
    else:
...
def get_digits(string):
    """Extract all digits from a string and return them as an integer."""
    return int("".join([c for c in string if c.isdigit()]))

def read_cif_bonds(cif):
    """Read bond endpoints and image offsets from a CIF bond loop.

    Returns empty lists when no `_ccdc_geom_bond_type` section is present.
    """
    ...
    bond_start = (
        int(np.nonzero(["_ccdc_geom_bond_type" in lin for lin in lines])[0][0]) + 1
    )
    from_index = []
    to_index = []
    to_jimage = []
